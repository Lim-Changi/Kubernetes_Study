...
Pod 수를 자동으로 조절하여 애플리케이션의 가용성과 성능을 최적화하는 리소스  
HPA는 CPU, 메모리, 커스텀 지표 등 다양한 지표를 기반으로 Pod 의 수를 동적으로 확장하거나 축소  
(역으로 Vertical Scale 이라 함은 보통 하드웨어적인 메모리나 CPU 성능을 늘리는 것을 통칭)
...
- Logging Solutions (Tool)
  - Cluster Level
    - Metrics Server -> K8s 자체 제공 [[Github]](https://github.com/kubernetes-sigs/metrics-server)
      - 메모리에 저장하는 방식이라 히스토리 관리는 안됨
    - ElasticSearch
    - Fluentd & Kibana EFK
